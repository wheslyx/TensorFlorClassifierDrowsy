...
vs = VideoStream(src=0).start() # instead of using cap = cv2.VideoCapture(0) streaming library is used
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        while Success:
            ...
            frame_start = vs.read() # instead of ret, image_np = cap.read() video stream is used
            frame = cv2.resize(frame_start,(711,400)) #frame = imutils.resize(frame_start, width=400) #each frame is resized to a width of 400 pixels, height 225 Image ratio is 0.5
            frame = frame[ : , 11:691]
            ...
            print("Time of Inference: " + str(time.perf_counter()))
